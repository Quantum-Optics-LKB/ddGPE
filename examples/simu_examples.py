...
parameters = [
    ...
    ("eV_to_J", cte.eV_to_J),
    ("n_cav", cte.n_cav),
    ("omega_exc (div by hbar)", cte.omega_exc * cte.h_bar),
    ("omega_cav (div by hbar)", cte.omega_cav * cte.h_bar),
    ("gamma_exc (div by hbar)", cte.gamma_exc * cte.h_bar),
    ("gamma_cav (div by hbar)", cte.gamma_cav * cte.h_bar),
    ("g0 (div by hbar)", cte.g0 * cte.h_bar),
    ("rabi (div by 2hbar)", cte.rabi * 2 * cte.h_bar),
    ("k_z", cte.k_z),
    ("detuning (div by hbar)", detuning * cte.h_bar),
    ("F_pump", F_pump),
    ("F_probe", F_probe),
    ("t_min", t_min),
    ("t_max", t_max),
...
(
    ...
    hopfield_coefs,
    F_t,
) = af.load_raw_data(folder, path_ic)
den_reservoir_txy = np.load(folder + "/raw_arrays/mean_den_reservoir_t_x_y.npy")
LP_t_x_y, LP_w_kx_ky = af.polariton_fields(
    cav_field_txy, exc_field_txy, hopfield_coefs, only_LP=True
)
LP_stat_x_y, LP_stat_kx_ky = af.stationary_polariton_fields(
    cav_stationary_xy, exc_stationary_xy, hopfield_coefs, only_LP=True
)
...
af.plot_gnLP_vs_I(
    ...
    C02,
    h_bar=h_bar,
    detuning=detuning * h_bar,
    theoretical=False,
)  # theoretical = False because it does not work yet aha
...
ax1.plot(
    time_plot,
    (g_LP * avg_density.get() + g0 * avg_den_res),
    label="$gn_{LP}(0,0)+g0n_{res}$",
    color="g",
)
