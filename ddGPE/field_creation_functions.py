...
def tophat(F_laser_r: cp.ndarray, R: cp.ndarray, radius: float = 75):
...
def gaussian(F_laser_r: cp.ndarray, R: cp.ndarray, radius=75):
    ...
    """
    F_laser_r[..., :, :] = F_laser_r[..., :, :] * cp.exp(
        -R[:, :] ** 2 / radius**2
    )  # identifying with Gaussian distribution, radius is sqrt(2)*std_deviation
    profile = "Gaussian, radius = " + str(radius) + " ; "
    return profile


def ring(F_probe_r: cp.ndarray, R: cp.ndarray, radius: float, delta_radius: float):
...
def plane_wave(F_laser_r: cp.ndarray, XX: cp.ndarray, kx=0.5):
...
def shear_layer(F_laser_r: cp.ndarray, XX: cp.ndarray, YY: cp.ndarray, kx: float = 1):
...
def radial_expo(F_probe_r: cp.ndarray, R: cp.ndarray, THETA: cp.ndarray, m, p):
    ...
    """
    F_probe_r[..., :, :] = (
        F_probe_r[..., :, :] * cp.exp(1j * p * R[:, :]) * cp.exp(1j * m * THETA[:, :])
    )
...
def vortex_beam(
    ...
    F_laser_r[..., :, :] = (
        F_laser_r[..., :, :]
        * cp.exp(1j * C * THETA[:, :])
        * cp.tanh(R[:, :] / inner_waist) ** C
    )
    profile = (
        "Vortex beam, C = " + str(C) + ", inner_waist = " + str(inner_waist) + " ; "
    )
    return profile
...
def transonic_phase(
    ...
    """
    phase = (k_down - k_up) / 2 * wh * cp.log(cp.cosh((YY - xh) / wh)) + (
        k_up + k_down
    ) / 2 * YY
    F_laser_r[..., :, :] = F_laser_r[..., :, :] * cp.exp(1j * phase[:, :])
    profile = (
...
def to_turning_point(F_laser_t: cp.ndarray, time: cp.ndarray, t_up=400, t_down=400):
    ...
    """
    F_laser_t[time < t_up] = 3 * cp.exp(
        -(((time[time < t_up] - t_up) / (t_up / 2)) ** 2)
    )
    F_laser_t[time >= t_up] = 1 + 2 * cp.exp(
        -(((time[time >= t_up] - t_up) / t_down) ** 2)
    )
    profile = (
        "Time profile: to turning point, t_up = "
        + str(t_up)
        + ", t_down = "
        + str(t_down)
        + " "
...
def turn_on(F_laser_t: cp.ndarray, time: cp.ndarray, t_up=200):
...
def wave_packet(
    ...
    Args:
        ...
        t_plateau (int, optional): width of the wave packet. Defaults to 10.
    """
    F_probe_t[time < t_up] = cp.exp(
        -1 * (time[time < t_up] - t_up) ** 2 / (t_up / 2) ** 2
    )
    F_probe_t[(time >= t_up) & (time <= t_up + t_plateau)] = 1
    F_probe_t[time > t_up + t_plateau] = cp.exp(
        -1
        * (time[time > t_up + t_plateau] - (t_up + t_plateau)) ** 2
        / (t_down / 2) ** 2
    )
    profile = (
        "Time profile: wave_packet, t_up = "
        + str(t_up)
        + ", t_down = "
        + str(t_down)
        + ", t_plateau = "
...
def tempo_probe(
    F_probe_t: cp.ndarray, omega_probe: float, t_probe, t_probe_end, time: cp.ndarray
):
...
def linear_ramp(
    ...
    F_probe_t[..., time < t_probe] = 0
    profile = (
        "Time profile: linear_ramp, omega_start = "
        + str(omega_start)
        + ", omega_end = "
        + str(omega_end)
        + ", t_probe = "
...
def step_ramp(
    ...
    profile = (
        "Time profile: step_ramp, omega_start = "
        + str(omega_start)
        + ", omega_end = "
        + str(omega_end)
        + ", omega_resol = "
