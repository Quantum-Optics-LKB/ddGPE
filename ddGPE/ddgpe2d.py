...
class ggpe:
    ...
    def __init__(
        ...
        self.X, self.delta_X = cp.linspace(
            -Lx / 2, Lx / 2, Nx, retstep=True, dtype=np.float64
        )
        self.Y, self.delta_Y = cp.linspace(
            -Ly / 2, Ly / 2, Ny, retstep=True, dtype=np.float64
        )
        ...
        # Energies
        self.omega_LP_0 = (self.omega_exc + self.omega_cav) / 2 - 0.5 * cp.sqrt(
            (self.omega_exc - self.omega_cav) ** 2 + 4 * self.rabi**2
        )
        ...
        self.omega[1, :, :] = (
            self.omega_cav * (cp.sqrt(1 + (self.Kxx**2 + self.Kyy**2) / self.k_z**2))
            - self.omega_pump
        )
        ...
        omega_max = max(
            ...
            * (
                np.sqrt(
                    1
                    + (
                        cp.fft.fftshift(self.Kx)[0] ** 2
                        + cp.fft.fftshift(self.Ky)[0] ** 2
                    )
                    / k_z**2
                )
            )
            - self.omega_pump,
            omega_exc - self.omega_pump,
        )
        ...
        self.omega_list = (
            2 * cp.pi * cp.fft.fftshift(cp.fft.fftfreq(self.n_frame, self.dt_frame))
        )
        ...
        self.omega_up = 0.5 * (
            self.omega[1, :, :]
            + self.omega[0, :, :]
            - 0.5 * 1j * (self.gamma[1, :, :] + self.gamma[0, :, :])
            + cp.sqrt(
                (
                    self.omega[1, :, :]
                    - self.omega[0, :, :]
        ...
        self.hopfield_coefs = cp.zeros(
            (2, self.Nx, self.Ny), dtype=np.complex64
        )  # self.hopfield_coefs[0,:,:]=Xk and self.hopfield_coefs[1,:,:]=Ck
        self.hopfield_coefs[1, :, :] = cp.sqrt(
            (
                cp.sqrt(
                    (self.omega[1, :, :] - self.omega[0, :, :]) ** 2 + 4 * self.rabi**2
                )
                - (self.omega[1, :, :] - self.omega[0, :, :])
            )
            / (
                2
                * cp.sqrt(
        ...
        # # Optional: min intensity (squared modulus)
        ...
        id_x_1, id_x_2 = cp.ones(self.XX.shape, dtype=cp.complex64), cp.ones(
            self.YY.shape, dtype=cp.complex64
        )
        A = cp.exp(
            -0.5
            * (
                cp.multiply(cp.transpose(id_x_1), self.YY + self.Ly / 2) ** 2
                / delta_gamma_2**2
            )
        ) + cp.exp(
            -0.5
            * (self.Ly - (cp.multiply(cp.transpose(id_x_1), self.YY + self.Ly / 2)))
            ** 2
            / delta_gamma_2**2
        )
        B = cp.exp(
            -0.5
            * (
                cp.multiply(cp.transpose(self.X + self.Lx / 2), id_x_2) ** 2
                / delta_gamma_1**2
            )
        ) + cp.exp(
            -0.5
            * (self.Lx - (cp.multiply(cp.transpose(self.X + self.Lx / 2), id_x_2))) ** 2
            / delta_gamma_1**2
        )
    ...
    def split_step(
        ...
        self.kernels.non_linearity(
            phi_exc, phi_cav, den_reservoir, self.dt, self.g0, self.X02
        )
        if self.apply_reservoir:
            self.kernels.reservoir_losses(
                den_reservoir,
                phi_exc,
                phi_cav,
                self.dt,
        ...
        # Fourier space
        plan_fft.fft(
            phi_exc, phi_exc, cp.cuda.cufft.CUFFT_FORWARD
        )  # should only transform the last 2 arrays
        plan_fft.fft(phi_cav, phi_cav, cp.cuda.cufft.CUFFT_FORWARD)

        self.kernels.linear_step(
            phi_exc,
            phi_cav,
            phi_up,
            phi_lp,
            self.propagator_diag[0, :, :],
            self.propagator_diag[1, :, :],
            self.hopfield_coefs[0, :, :],
            self.hopfield_coefs[1, :, :],
        )
        ...
        if k * self.dt >= t_noise:
            rand1 = cp.random.normal(
                loc=0, scale=self.dt, size=(self.Nx, self.Ny), dtype=np.float64
            ) + 1j * cp.random.normal(
                loc=0, scale=self.dt, size=(self.Nx, self.Ny), dtype=np.float64
            )
    ...
    def evolution(
        ...
        if save and stationary == 0:
            self.mean_cav_t_x_y = cp.zeros(
                self.phi[1, ..., :, :].shape[0:-2] + (self.n_frame, self.Nx, self.Ny),
                dtype=np.complex64,
            )
            self.mean_exc_t_x_y = cp.zeros(
                self.phi[0, ..., :, :].shape[0:-2] + (self.n_frame, self.Nx, self.Ny),
                dtype=np.complex64,
            )
            self.mean_den_reservoir_t_x_y = cp.zeros(
                self.den_reservoir[..., :, :].shape[0:-2]
                + (self.n_frame, self.Nx, self.Ny),
                dtype=np.complex64,
            )
            self.mean_cav_x_y_stat = cp.zeros(
                self.phi[1, ..., :, :].shape, dtype=np.complex64
            )
            self.mean_exc_x_y_stat = cp.zeros(
                self.phi[0, ..., :, :].shape, dtype=np.complex64
            )
            self.mean_den_reservoir_x_y_stat = cp.zeros(
                self.den_reservoir[..., :, :].shape[0:-2]
                + (self.n_frame, self.Nx, self.Ny),
                dtype=np.complex64,
            )
        ...
        if save_fields_at_time > 0:
            self.mean_cav_t_save = cp.zeros(
                self.phi[1, ..., :, :].shape, dtype=np.complex64
            )
            self.mean_exc_t_save = cp.zeros(
                self.phi[0, ..., :, :].shape, dtype=np.complex64
            )
            self.mean_den_reservoir_t_save = cp.zeros(
                self.den_reservoir[..., :, :].shape, dtype=np.complex64
            )
            self.F_t = cp.zeros(self.n_frame, dtype=np.float32)
            save_fields = 0
        ...
        for k in tqdm(range(len(self.time))):
            self.split_step(
                self.phi, self.phi_pol, self.den_reservoir, plan_fft, k, self.t_noise
            )
            if k * self.dt >= self.t_stationary and stationary < 1:
                print(
                    "Saving stationary state: k = "
                    + str(k)
                    + ", t = "
                    + str(k * self.dt)
                    + " ps"
            ...
            if k * self.dt >= save_fields_at_time and save_fields < 1:
                ...
                self.mean_den_reservoir_t_save = self.den_reservoir[..., :, :]
                print(
                    "Saving fields state: k = "
                    + str(k)
                    + ", t = "
                    + str(k * self.dt)
                    + " ps"
            ...
            if k * self.dt >= self.t_obs and save:
                ...
                if r_t >= self.dt_frame:
                    ...
                    self.mean_den_reservoir_t_x_y[..., i_frame, :, :] = (
                        self.den_reservoir[..., :, :]
                    )
